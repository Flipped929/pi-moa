...
captain（强模型, pi 主会话）── 拆片/派活/抽查/裁决
 ...
 └─ devil     (跨家族模型, 只读)       ← 异构魔鬼代言人
      ...
      ▼
 黑板 .pi/moa/<task>/  ← 全程留痕，Navigator 治理面异步审计
```

安全硬边界（scope-guard，物理拦截而非 prompt 自觉）：
敏感路径禁写 · 子代理禁写工作目录外（含 bash 绕行拦截）· 出网 payload PII 自动打码 · 会话 token 预算告警
...
预期：captain 建黑板、并行派活、critic 找出 2 个必崩 bug、终稿落盘 `.pi/moa/`。

## 模型 roster（换成你手里的模型）

默认：`executor/analyst/critic = deepseek-v4-flash`，`devil/captain = kimi-k3`。
...
```
extensions/   pi 扩展（moa-mode 调度 / scope-guard 安全 / subagent 派生）
agents/       角色定义（executor/analyst/critic/devil）
